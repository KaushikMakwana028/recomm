...
public function get_profile($id = null)
{
    ...
    if (!$id) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'User ID is required', 'data' => null
        ]));
    }
    ...
    if (!$user) {
        return $this->output->set_status_header(404)->set_output(json_encode([
            'status' => false, 'code' => 404, 'message' => 'User not found', 'data' => null
        ]));
    }
    ...
    if ((int)$decoded->data->id !== (int)$id) {
        return $this->output->set_status_header(403)->set_output(json_encode([
            'status' => false, 'code' => 403, 'message' => 'You can only access your own profile', 'data' => null
        ]));
    }
...
public function update_stock()
{
    ...
    if (!$product) {
        return $this->output->set_status_header(404)->set_output(json_encode([
            'status' => false, 'code' => 404, 'message' => 'Product not found', 'data' => null
        ]));
    }
...
public function get_offers()
{
    ...
    foreach ($offers as $offer) {
        if ($offer->is_active == 1) $active++; else $inactive++;
...
$is_active       = !empty($json['is_active']) ? 1 : 0;
    ...
    if (empty($name) || empty($offer_type) || empty($discount_value) || empty($min_amount) || empty($start_date) || empty($end_date)) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'All fields are required', 'data' => null
        ]));
    }

    if (!in_array($offer_type, ['flat', 'percentage'])) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'Invalid offer type', 'data' => null
        ]));
    }

    if ($discount_value <= 0 || $min_amount <= 0) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'Discount and minimum amount must be greater than 0', 'data' => null
        ]));
    }

    if (strtotime($start_date) >= strtotime($end_date)) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'End date must be after start date', 'data' => null
        ]));
    }
...
public function delete_offer($id = null)
{
    ...
    if (!$id) {
        return $this->output->set_status_header(422)->set_output(json_encode([
            'status' => false, 'code' => 422, 'message' => 'Offer ID is required', 'data' => null
        ]));
    }
    ...
    if (!$offer) {
        return $this->output->set_status_header(404)->set_output(json_encode([
            'status' => false, 'code' => 404, 'message' => 'Offer not found', 'data' => null
        ]));
    }
